...
import {
  ...
  Menu, X, Layers, Tag, RefreshCw, Mail, Image, MessageSquareQuote,
} from "lucide-react";
...
type Tab =
  | "dashboard" | "leads" | "clients" | "hosting" | "bookings" | "offers"
  | "diagnostics" | "settings" | "projects" | "finance" | "billing"
  | "invoice_settings" | "offres_packs" | "options_tarifs" | "abonnements"
  | "resend" | "portfolio" | "testimonials";
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        {SIDEBAR_SECTIONS.map((section) => (
          <div key={section.label} className="mb-4">
            {!sidebarCollapsed && (
              <p className="text-[10px] font-semibold text-muted-foreground/60 uppercase tracking-[0.15em] px-5 mb-2">
                {section.label}
              </p>
            )}
                  ...
                  >
                    ...
                    {sidebarCollapsed && badge && (
                      <span className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full ${
                        badge.type === "destructive" ? "bg-destructive" : badge.type === "warning" ? "bg-conversion" : "bg-primary"
                      }`} />
                    )}
                  </button>
                );
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
            {hostingAlerts > 0 && (
              <button
                ...
                className="hidden md:flex items-center gap-2 text-xs bg-destructive/10 text-destructive px-3 py-1.5 rounded-full hover:bg-destructive/20 transition-colors animate-pulse"
              >
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto">
          ...
          {tab === "portfolio"        && <AdminPortfolioTab />}
